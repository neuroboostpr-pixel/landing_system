...
**ПРОТОТИП = ТОЛЬКО СТРУКТУРА (reference-driven §1.1).** Из прототипа берём
ИСКЛЮЧИТЕЛЬНО структуру: блоки, порядок, тексты, кнопки. **Цвета / шрифты /
стиль / тему прототипа ИГНОРИРУЕМ.** Прототип бывает wireframe, цветной,
светлый, тёмный — его визуал не имеет значения и НЕ переносится. Светлый
прототип ≠ светлый сайт. Вид берётся из дизайн-системы (этап 05), не отсюда.
Если у клиента несколько прототипов — это варианты СТРУКТУРЫ, к каждому
применяется одна дизайн-система.
