...
**ПРАВИЛО: каждый шаг — отдельное сообщение с явным ожиданием ответа.**
Не переходить к следующему шагу пока не получен один из двух вариантов:
1. Явный ответ-пропуск: "пропустить", "skip", "нет", "позже", "не нужно" (любой синоним)
2. Подтверждение что материалы положены: "готово", "done", "ok", "положил(а)", или пришли сами файлы/пути

Нельзя обработать материалы молча из предыдущего сообщения и пропустить шаг — даже если файлы уже понятны из контекста. Каждый шаг должен быть показан явно.
