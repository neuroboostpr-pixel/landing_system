...
- **KSES экранирует вложенные блоки → сырой код вместо карточек.** При
  сохранении `page-content` через `wp_update_post()` (или любой путь, где
  активны content-фильтры) вложенные `<!-- wp:lazyblock/* -->` внутри
  section-card блока (напр. `brand-card` внутри `brands`) превращаются в
  `&lt;!-- wp:... -->`. WordPress перестаёт парсить их как блоки и выводит
  **сырой текст комментария** на странице (симптом: на лендинге видны
  `<!-- wp:lazyblock/brand-card {...} -->`, `"logo_html": "nnnn"`; контент
  в БД обрезан). Изолированный `do_blocks()` блока при этом рендерит ОК —
  значит дело не в `block.php`. **Фикс:** писать `post_content` напрямую
  `$wpdb->update($wpdb->posts, …)` минуя фильтры, либо снять kses
  (`kses_remove_filters()`) перед сохранением. `wp post create/update`
  через wp-cli в admin-контексте обычно безопасен, но при программной
  записи через API — всегда проверяй, что в БД лежит `<!-- wp:` а не `&lt;`.

- **Многострочный inline-SVG в атрибуте блока.** `logo_html` (и любой
  textarea-атрибут) с реальными переносами строк внутри JSON блок-комментария
  ломает block-parser и провоцирует KSES-экранирование выше. Block-комментарий
  Gutenberg обязан быть однострочным. `generate-page-content.py::_collapse_ws`
  схлопывает `\n`/`\r` в пробел во всех строковых атрибутах (для SVG/HTML
  переносы незначимы). Не откатывать.

- **Абсолютные пути с подпапкой деплоя.** Если тест-деплой был в подпапку
  (`site.beget.tech/hibridcars`), в HTML могут захардкодиться `/hibridcars/wp-content/…`
  и `href="/hibridcars/"`. На проде (корень домена) → 404 картинок и битые
  ссылки между страницами. Пути в теме держать корне-относительными
  (`/wp-content/…`, `href="/"`, `href="/li-auto/"`) или через
  `get_template_directory_uri()`. Проверка перед деплоем: `grep -r "/<подпапка>/" wp-theme/`.
