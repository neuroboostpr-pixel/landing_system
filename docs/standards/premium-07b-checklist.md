...
> **v2 (2026-06-12, reference-driven flow).** Обязательный минимум проверяет
> `scripts/verify-composed-premium.sh` (v2): токены в `:root`, `clamp()`,
> движение, hover, `prefers-reduced-motion`, production-голова (OG, favicon,
> theme-color, шрифты) и ЗАПРЕТ эмодзи в заголовках. Интерактивные фичи ниже
> (parallax/slider/lightbox/gradient text и т.п.) — **рекомендации**: ставятся
> под потребность места ([design-elements-rules.md](design-elements-rules.md)),
> а не обязательным набором. **Премиум ≠ тёмный фон** — премиальность даёт
> композиционная глубина (спека reference-driven flow §3).
...
- [ ] `07_ПРОТОТИП/prototype.md` — дословная структура и тексты (канон)
...
## 12.5. Панель мудов в composed.html — ОБЯЗАТЕЛЬНО (все проекты)

`composed.html` ОБЯЗАН содержать **панель-переключатель мудов** (дизайн-систем),
чтобы видеть лендинг в разных палитрах ДО публикации (превью на тестовом сайте
для согласования с клиентом). Это тот же механизм, что WP-плагин
`lp-preview-panel`, но доступный уже на этапе composed.

**Требования:**
- [ ] **Единый источник мудов:** composed-панель и WP-плагин читают ОДНИ И ТЕ ЖЕ
  муды (палитры из `palettes.css` / `palettes.yaml` проекта + системные
  `block-library/_styles/`). Не дублировать наборы — иначе превью и прод разойдутся.
- [ ] **Состав мудов:** референсы проекта (см. `03_РЕФЕРЕНСЫ/references-index.md`,
  канон + кандидаты) + 6 системных style-moods.
- [ ] **Механика:** каждый муд = набор значений токенов; клик подменяет `:root`
  (или `body.theme-<id>`) → весь лендинг перекрашивается. Работает ТОЛЬКО при 100%
  токенизации (0 прямых цветов вне `:root` — см. раздел 2, иначе муд «протекает»).
- [ ] **Флаг on/off:** панель включается/выключается одним атрибутом
  (напр. `data-preview="on"`). Едет на тестовый сайт включённой (для согласования),
  ПЕРЕД продом — выключается. Дефолт-муд фиксируется = канон из references-index.
- [ ] **Позиция:** fixed, угол экрана, не перекрывает контент, сворачиваемая.
- [ ] Переключение НЕ ломает вёрстку (тёмный муд → светлый текст и наоборот, всё
  через семантические токены bg/text/accent, не «orange/beige»).

См. `reference-driven-flow-spec.md §2.5`,
`2026-05-12-preview-panel-and-palette-library-design.md`.
...
- [ ] **Панель мудов встроена** (раздел 12.5): переключить муды — лендинг перекрашивается, вёрстка не ломается, тёмный муд читаем; флаг on/off работает
