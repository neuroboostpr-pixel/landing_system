...
Фаза 0 (словарь) ── фундамент для всех
   └─> Фаза 1 (variant в meta + миграция 154 блоков) ── самая трудоёмкая, ручная доводка
          └─> Фаза 2 (catalog читает variant)
                 ├─> Фаза 3 (удаление cta+gallery, консервативно)
                 ├─> Фаза 4 (галерея 2 комбобокса, RU)
                 └─> Фаза 5 (wireframe variant-aware, убрать score>0)
