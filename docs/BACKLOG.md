...
### B21. `/landing-delete <slug>` — удаление проекта

- **Зачем:** сейчас нет безопасного способа удалить проект не трогая другие папки. Приходится вручную.
- **Что нужно:** slash-команда + скрипт `scripts/landing-delete.sh <slug>`. Проверяет что папка `~/Lendings/<slug>/` существует, показывает список файлов, просит явного подтверждения, удаляет. Не трогает ничего за пределами `~/Lendings/<slug>/`.
- **Размер:** малый (~2ч).
- **Зависимости:** нет.

### B22. `/landing-rename <old> <new>` — переименование проекта

- **Зачем:** нет способа переименовать проект без ручного mv + правки `.landing-state.yaml` и других файлов с путями.
- **Что нужно:** slash-команда + скрипт `scripts/landing-rename.sh <old> <new>`. Переименовывает папку, обновляет `slug` во всех yaml-файлах проекта, проверяет что новый slug свободен.
- **Размер:** малый (~3ч).
- **Зависимости:** нет.
