...
Без стилей проекта                         + единый нейтральный lo-fi стиль
...
- Рендерит template.html каждого блока с единым нейтральным CSS-сбросом (серые тона, system-ui)
- Подставляет реальные тексты из prototype.yaml в слоты (`data-slot="headline"` → реальный headline)
...
**Что нужно:**
Два режима wireframe:
1. **Lo-fi mode** (дефолт): полностью бесстилевой — только серые прямоугольники с разметкой, зато реальные тексты из `prototype.yaml`. Нет CSS из block-library.
2. **Styled mode**: единый нейтральный стиль для всех блоков (один шрифт, одна палитра серых), реальные тексты.

В обоих режимах: `inject-content.py` подставляет тексты из `prototype.yaml` перед рендером wireframe.
...
### B34. Новая таксономия типов блоков + умный импортёр

**Что происходит сейчас:**
- 9 типов блоков (`hero, features, trust, social-proof, process, pricing, faq, cta, quiz`) — `trust` покрывает совершенно разные вещи: манифест, гарантии, цифры, партнёры
- 190 блоков в библиотеке, большинство — автоматически загруженный мусор с неинформативными названиями типа `"Контактная полоса с крупным номером, подписью и маленькой оранжевой ссылкой"` (цвета в описании бесполезны)
- Импортёр (`scripts/import-blocks/import-from-url.sh`) добавляет блоки без проверки дублей по layout-паттерну
- `display_name_ru` описывает стиль, а не структуру

**Что нужно:**

**Часть 1 — Новая таксономия (14 типов):**
`header`, `hero`, `features`, `manifesto`, `stats`, `guarantees`, `partners`, `social-proof`, `process`, `gallery`, `pricing`, `faq`, `cta`, `lead-form`

Убрать `trust` (слишком широкий) и `quiz` (переименовать в `lead-form` для простых форм, `quiz` оставить только для пошаговых воронок).

**Часть 2 — Чистка библиотеки:**
Оставить только ~30 эталонных блоков покрывающих все layout-паттерны. Критерий сохранения — уникальная структура (grid, flex, split, stacked и т.д.), а не стиль.

**Часть 3 — Умный импортёр (новый скил `/landing-import-blocks`):**
- Принимает скриншот или URL
- Codex извлекает структуру блоков (layout-паттерн, слоты, тип)
- Проверяет что в библиотеке нет блока с идентичным layout-паттерном в той же категории
- Если уникальный — добавляет с именем формата `hero-split-photo-right` (структура, не стиль)
- Если дубль — показывает похожий блок и спрашивает подтверждение

**Часть 4 — Переименование display_name_ru:**
Формат: `"[Категория]: [структура элементов]"` — например:
- ❌ `"Контактная полоса с крупным номером, подписью и маленькой оранжевой ссылкой"`
- ✅ `"Контакты: телефон крупно + подпись + ссылка"`
- ❌ `"Сетка 4×2 карточек с принципами работы"`
- ✅ `"Features: 4×2 карточки с нумерацией"`
